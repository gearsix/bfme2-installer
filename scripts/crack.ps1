...
"`t Copying crack file...`n"
#copy crack dir
Copy-Item $DIR_START"\game_files\"$TARGET"\game.dat" $DIR_INSTALL

"`t Writing options.ini...`n"
if (!(Test-Path $DIR_APPDATA))
{
	New-Item -ItemType directory -Path $DIR_APPDATA | Out-Null
}
Copy-Item $DIR_START"\game_files\options.ini" $DIR_APPDATA | Out-Null

"`t Successfully applied pirate crack!"